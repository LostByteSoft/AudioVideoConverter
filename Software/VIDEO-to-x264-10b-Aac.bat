...
echo Select and file name
	...
	echo/
	echo Save file name will be : "%name%-x264.10b-Aac.mkv"
	timeout /t 1
	echo/
...
echo Convert
	echo/
	ffmpeg -loglevel error -v error -stats -i "%file%" -c:v libx264 -crf 22 -preset faster -tune fastdecode -pix_fmt yuv420p10le -c:a aac "%name%-x264.10b-Aac.mkv"
	echo/
